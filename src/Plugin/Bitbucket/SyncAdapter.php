...
class SyncAdapter implements SyncAdapterInterface {
	...
	private function getAllProjects( Remote $remote ) {
		...
		$pageN    = 1;
		while ( true ) {
			$curr_proj = json_decode( $response->getContent(), true )['values'];
			$projects  = array_merge( $projects, $curr_proj );
			if ( ! $page->hasNext() ) {
				break;
			}
	...
	public function enableHook( Package $package ) {
		...
		try {
			...
			$hookUrl  = str_replace( '.local', '.com', $hookUrl );

			$response = $Hook->create( $account, $repoName, [
				'description' => 'Rebuild on Terramar Packagist',
				'url'         => $hookUrl,
				'active'      => true,
				'events'      => [ 'repo:push' ],
			] );
